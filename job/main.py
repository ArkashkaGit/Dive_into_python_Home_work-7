...
"""
решил поменять один пункт из задания так как посчитал его не логичным
- Принимать параметр количество цифр в порядковом номере.
при большом количестве файлов может быть не актуален данный пункт..
поменял на стартовое число счётчика
"""
import os

# 1 - новое имя, 2 - начальное число порядковых номеровб 3 - выбор элементов по расширению
